...
class FwdStackOp(FwdOpbase):
    ...
    def _jacobian_impl(self,**kwargs):
        ...
        for stack_id,fwdop in enumerate(self._fwdops):
            ...
            if stack_id>0:
                prev_dims=[fwd._unkdim for fwd in self._fwdops[:stack_id]]
                if fwdop._unkdim in prev_dims:
                    jacobian_i=jacobian_i.rename({fwdop._unkdim:f"{fwdop._unkdim}_{stack_id}"})
                    fwdop._unkdim=f"{fwdop._unkdim}_{stack_id}"
